...
**`vina_all_poses_sort`**
* This script automatically scans the current directory for Vina-GPU output files (`.log`), extracts the best binding affinity scores (All Modes), and sorts the results from the lowest (best) to highest energy. It prints a clean table to the terminal and simultaneously saves it as a `results.txt` file in the same directory.
